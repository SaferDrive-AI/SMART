...
def interplating_polyline(polylines, heading, distance=0.5, split_distace=5):
    ...
    # plt.plot(polylines[:, 0], polylines[:, 1], "r")
    # plt.axis("equal")
    # plt.savefig("tmp.jpg")
    # plt.close()
    # # Plot all polylines in a single figure
    # plt.figure()
    # if len(polylines_list) > 1:
    #     for polyline_group in polylines_list:
    #         polyline_group = np.array(polyline_group)
    #         plt.plot(polyline_group[:, 0], polyline_group[:, 1])
    #     plt.axis("equal")
    #     plt.savefig("polylines_split.jpg")
    #     plt.close()
    new_x_list = []
    new_y_list = []
...
class TokenProcessor:
    ...
    def __init__(self, token_size):
        ...
        self.agent_token_path = os.path.join(
            module_dir, f"tokens/cluster_frame_5_{token_size}.pkl"
        )
        self.map_token_traj_path = os.path.join(
            module_dir, "tokens/map_traj_token5.pkl"
        )
        self.noise = False
        self.disturb = False
        self.shift = 5
    ...
    def clean_heading(self, data):
        ...
        for i in range(n_frames - 1):
            change_needed = (torch.abs(heading_diff[:, i : i + 1]) > 1.0) & valid_pairs[
                :, i : i + 1
            ]

            heading[:, i + 1][change_needed.squeeze()] = heading[:, i][
                change_needed.squeeze()
            ]

            if i < n_frames - 2:
                heading_diff_raw = heading[:, i + 1] - heading[:, i + 2]
                heading_diff[:, i + 1] = (
                    torch.remainder(heading_diff_raw + pi, 2 * pi) - pi
                )
    ...
    def tokenize_agent(self, data):
        ...
        interplote_mask = (
            data["agent"]["valid_mask"][:, self.current_step] == False
        ...
        if data["agent"]["velocity"].shape[-1] == 2:
            data["agent"]["velocity"] = torch.cat(
                [
                    data["agent"]["velocity"],
                    torch.zeros(
                        data["agent"]["velocity"].shape[0],
                        data["agent"]["velocity"].shape[1],
                        1,
                    ),
                ],
                dim=-1,
        ...
        data["agent"]["position"][interplote_mask, self.current_step - 1, :3] = (
            data["agent"]["position"][interplote_mask, self.current_step, :3]
            - vel * 0.1
        )
        data["agent"]["valid_mask"][
            interplote_mask, self.current_step - 1 : self.current_step + 1
        ] = True
        data["agent"]["heading"][interplote_mask, self.current_step - 1] = data[
            "agent"
        ]["heading"][interplote_mask, self.current_step]
        data["agent"]["velocity"][interplote_mask, self.current_step - 1] = data[
            "agent"
        ]["velocity"][interplote_mask, self.current_step]

        data["agent"]["type"] = data["agent"]["type"].to(torch.uint8)

        self.clean_heading(data)
        matching_extra_mask = (
            data["agent"]["valid_mask"][:, self.current_step] == True
        ) * (data["agent"]["valid_mask"][:, self.current_step - 5] == False)

        interplote_mask_first = (data["agent"]["valid_mask"][:, 0] == False) * (
            data["agent"]["position"][:, 0, 0] != 0
        )
        ...
        veh_token_index, veh_token_contour = self.match_token(
            veh_pos,
            veh_valid_mask,
            agent_heading[vehicle_mask],
            "veh",
            agent_category[vehicle_mask],
            matching_extra_mask[vehicle_mask],
        )
        ped_token_index, ped_token_contour = self.match_token(
            ped_pos,
            ped_valid_mask,
            agent_heading[ped_mask],
            "ped",
            agent_category[ped_mask],
            matching_extra_mask[ped_mask],
        )
        cyc_token_index, cyc_token_contour = self.match_token(
            cyc_pos,
            cyc_valid_mask,
            agent_heading[cyclist_mask],
            "cyc",
            agent_category[cyclist_mask],
            matching_extra_mask[cyclist_mask],
        )

        token_index = torch.zeros((agent_pos.shape[0], veh_token_index.shape[1])).to(
            torch.int64
        )
        ...
        token_contour = torch.zeros(
            (
                agent_pos.shape[0],
                veh_token_contour.shape[1],
                veh_token_contour.shape[2],
                veh_token_contour.shape[3],
            )
        )
        ...
        trajectory_token_veh = (
            torch.from_numpy(self.trajectory_token["veh"]).clone().to(torch.float)
        )
        trajectory_token_ped = (
            torch.from_numpy(self.trajectory_token["ped"]).clone().to(torch.float)
        )
        trajectory_token_cyc = (
            torch.from_numpy(self.trajectory_token["cyc"]).clone().to(torch.float)
        )

        agent_token_traj = torch.zeros(
            (agent_pos.shape[0], trajectory_token_veh.shape[0], 4, 2)
        )
        ...
        data["agent"]["token_heading"] = torch.arctan2(
            diff_xy[:, :, 1], diff_xy[:, :, 0]
        )
        ...
        vel = torch.cat(
            [
                token_pos.new_zeros(data["agent"]["num_nodes"], 1, 2),
                ((token_pos[:, 1:] - token_pos[:, :-1]) / (0.1 * self.shift)),
            ],
            dim=1,
        )
        vel_valid_mask = torch.cat(
            [
                torch.zeros(token_valid_mask.shape[0], 1, dtype=torch.bool),
                (token_valid_mask * token_valid_mask.roll(shifts=1, dims=1))[:, 1:],
            ],
            dim=1,
        )
        vel[~vel_valid_mask] = 0
        vel[data["agent"]["valid_mask"][:, self.current_step], 1] = data["agent"][
            "velocity"
        ][data["agent"]["valid_mask"][:, self.current_step], self.current_step, :2]
    ...
    def match_token(
        self, pos, valid_mask, heading, category, agent_category, extra_mask
    ):
        ...
        agent_token_src = agent_token_src.reshape(
            1, token_num * token_contour_dim, feat_dim
        ).repeat(agent_num, 0)
        token_last = token_last.reshape(
            1, token_num * token_contour_dim, feat_dim
        ).repeat(extra_mask.sum(), 0)
        token_index_list = []
        token_contour_list = []
        prev_token_idx = None
        ...
        for i in range(self.shift, pos.shape[1], self.shift):
            ...
            rot_mat[:, 1, 0] = -sin
            rot_mat[:, 1, 1] = cos
            agent_token_world = torch.bmm(
                torch.from_numpy(agent_token_src).to(torch.float), rot_mat
            ).reshape(agent_num, token_num, token_contour_dim, feat_dim)
            agent_token_world += prev_pos[:, None, None, :]

            cur_contour = cal_polygon_contour(
                cur_pos[:, 0], cur_pos[:, 1], cur_heading, width, length
            )
            agent_token_index = torch.from_numpy(
                np.argmin(
                    np.mean(
                        np.sqrt(
                            np.sum(
            ...
            token_contour_select = agent_token_world[
                torch.arange(agent_num), agent_token_index
            ]

            diff_xy = token_contour_select[:, 0, :] - token_contour_select[:, 3, :]

            prev_heading = heading[:, i].clone()
            prev_heading[valid_mask[:, i - self.shift]] = torch.arctan2(
                diff_xy[:, 1], diff_xy[:, 0]
            )[valid_mask[:, i - self.shift]]

            prev_pos = pos[:, i].clone()
            prev_pos[valid_mask[:, i - self.shift]] = token_contour_select.mean(dim=1)[
                valid_mask[:, i - self.shift]
            ]
        ...
        if not self.training:
            ...
            rot_mat[:, 1, 1] = cos
            agent_token_world = torch.bmm(
                torch.from_numpy(token_last).to(torch.float), rot_mat
            ).reshape(extra_mask.sum(), token_num, token_contour_dim, feat_dim)
            agent_token_world += prev_pos[:, None, None, :]
            ...
            token_contour_select = agent_token_world[
                torch.arange(extra_mask.sum()), agent_token_index
            ]
