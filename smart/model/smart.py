...
class SMART(pl.LightningModule):
    ...
    def training_step(self, data, batch_idx):
        ...
        loss = cls_loss
        self.log(
            "train_loss", loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=1
        )
        self.log(
            "cls_loss",
            cls_loss,
    ...
    def validation_step(self, data, batch_idx):
        ...
        if self.inference_token:
            ...
            if (
                batch_idx % 100 == 0
            ):  # Visualize every 100 batches to avoid too many plots
                self.visualize_trajectories(
                    gt=gt[eval_mask],
                    pred=pred_traj[eval_mask],
                    batch_idx=batch_idx,
                    max_samples=3,  # Limit number of samples to visualize
                )
    ...
    def visualize_trajectories(self, gt, pred, batch_idx, max_samples=3):
        """
        Visualize ground truth and predicted trajectories

        Args:
            gt: Ground truth trajectories [N, T, 2]
            pred: Predicted trajectories [N, K, T, 2] where K is number of predictions
        ...
        n_samples = min(max_samples, len(gt))

        for i in range(n_samples):
            plt.figure(figsize=(10, 10))

            # Plot ground truth trajectory
            gt_traj = gt[i].cpu().numpy()
            plt.plot(
                gt_traj[:, 0], gt_traj[:, 1], "b-", label="Ground Truth", linewidth=2
            )

            # Plot predicted trajectories
            pred_trajs = pred[i].cpu().numpy()  # [K, T, 2]
            for k in range(pred_trajs.shape[0]):
                plt.plot(
                    ...
                    "r--",
                    alpha=0.5,
                    label=f"Prediction {k+1}" if k == 0 else None,
                )

            # Add start and end markers
            plt.plot(gt_traj[0, 0], gt_traj[0, 1], "go", label="Start")
            plt.plot(gt_traj[-1, 0], gt_traj[-1, 1], "ro", label="End")

            plt.title(f"Trajectory Visualization (Batch {batch_idx}, Sample {i})")
            ...
            # Save plot
            save_dir = (
                os.path.join(self.logger.log_dir, "trajectory_vis")
                if self.logger
                else "trajectory_vis"
            )
            os.makedirs(save_dir, exist_ok=True)
            plt.savefig(os.path.join(save_dir, f"traj_batch{batch_idx}_sample{i}.png"))
            plt.close()
