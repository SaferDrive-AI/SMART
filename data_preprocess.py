...
if os.path.isdir(_processed_dir):
    _processed_file_names = [
        name for name in os.listdir(_processed_dir) if name.endswith(("pkl", "pickle"))
    ]
else:
    _processed_file_names = []
...
Lane_type_hash = {4: "BIKE", 3: "VEHICLE", 2: "VEHICLE", 1: "BUS"}
...
def get_agent_features(
    ...
    if split == "test":
        predict_mask[
            current_valid_mask | (agent_category == 2) | (agent_category == 3),
...
def get_map_features(map_infos, tf_current_light, dim=3):
    ...
    for lane_segment in lane_segments:
        ...
        polygon_type[lane_segment_idx] = _polygon_types.index(
            Lane_type_hash[lane_segment.type]
        )

        res = tf_current_light[tf_current_light["lane_id"] == str(lane_segment.id)]
        if len(res) != 0:
            polygon_light_type[lane_segment_idx] = _polygon_light_type.index(
                res["state"].item()
            )
        ...
        point_magnitude[lane_segment_idx] = torch.norm(
            torch.cat([center_vectors[:, :2]], dim=0), p=2, dim=-1
        )
...
def process_agent(
    ...
    new_agents_array = new_agents_array[new_agents_array[..., -1] == 1.0].reshape(
        -1, new_agents_array.shape[-1]
    )
    new_agents_array = new_agents_array[
        ..., [0, 1, 2, 3, 4, 11, 12, 13, 14, 15, 16, 17, 18, 19, 5, 6, 7, 8, 9, 10]
    ]
    new_agents_array = pd.DataFrame(data=new_agents_array, columns=columns)
    new_agents_array["object_type"] = new_agents_array["object_type"].apply(
        func=type_hash
    )
    new_agents_array["start_timestamp"] = new_agents_array["start_timestamp"].astype(
        int
    )
...
def decode_tracks_from_proto(tracks):
    ...
    track_infos["trajs"] = np.stack(
        track_infos["trajs"], axis=0
    )  # (num_objects, num_timestamp, 9)
    return track_infos
...
def decode_map_features_from_proto(map_features):
    ...
    for cur_data in map_features:
        ...
        if cur_data.lane.ByteSize() > 0:
            ...
            cur_info["type"] = (
                cur_data.lane.type + 1
            )  # 0: undefined, 1: freeway, 2: surface_street, 3: bike_lane
            cur_info["left_neighbors"] = [
                lane.feature_id for lane in cur_data.lane.left_neighbors
            ]

            cur_info["right_neighbors"] = [
                lane.feature_id for lane in cur_data.lane.right_neighbors
            ]
            ...
            cur_info["left_boundary_type"] = [
                x.boundary_type + 5 for x in cur_data.lane.left_boundaries
            ] #左边是实线还是虚线还是马路牙子
            cur_info["right_boundary_type"] = [
                x.boundary_type + 5 for x in cur_data.lane.right_boundaries
            ]

            cur_info["left_boundary"] = [
                x.boundary_feature_id for x in cur_data.lane.left_boundaries
            ]
            cur_info["right_boundary"] = [
                x.boundary_feature_id for x in cur_data.lane.right_boundaries
            ]
            cur_info["left_boundary_start_index"] = [
                lane.lane_start_index for lane in cur_data.lane.left_boundaries
            ]
            cur_info["left_boundary_end_index"] = [
                lane.lane_end_index for lane in cur_data.lane.left_boundaries
            ]
            cur_info["right_boundary_start_index"] = [
                lane.lane_start_index for lane in cur_data.lane.right_boundaries
            ]
            cur_info["right_boundary_end_index"] = [
                lane.lane_end_index for lane in cur_data.lane.right_boundaries
            ]
            ...
            cur_polyline = np.stack(
                [
                    np.array([point.x, point.y, point.z, global_type, cur_data.id])
                    for point in cur_data.lane.polyline
                ],
                axis=0,
            )
            cur_polyline = np.concatenate(
                (cur_polyline[:, 0:3], cur_polyline[:, 3:]), axis=-1
            )
            ...
            cur_polyline = np.stack(
                [
                    np.array([point.x, point.y, point.z, global_type, cur_data.id])
                    for point in cur_data.road_line.polyline
                ],
                axis=0,
            )
            cur_polyline = np.concatenate(
            ...
            cur_polyline = np.stack(
                [
                    np.array([point.x, point.y, point.z, global_type, cur_data.id])
                    for point in cur_data.road_edge.polyline
                ],
                axis=0,
            )
            cur_polyline = np.concatenate(
            ...
            cur_polyline = np.array(
                [point.x, point.y, point.z, global_type, cur_data.id]
            ).reshape(1, 5)
            if cur_polyline.shape[0] <= 1:
                continue
            ...
            cur_polyline = np.stack(
                [
                    np.array([point.x, point.y, point.z, global_type, cur_data.id])
                    for point in cur_data.crosswalk.polygon
                ],
                axis=0,
            )
            cur_polyline = np.concatenate(
            ...
            cur_polyline = np.stack(
                [
                    np.array([point.x, point.y, point.z, global_type, cur_data.id])
                    for point in cur_data.speed_bump.polygon
                ],
                axis=0,
            )
            cur_polyline = np.concatenate(
...
def wm2argo(file, dir_name, output_dir):
    ...
    for cnt, data in enumerate(dataset):
        ...
        new_agents_array = process_agent(
            track_info, tracks_to_predict, sdc_track_index, scenario_id, 0, 91
        )  # mtr2argo
        data = dict()
        ...
        data["agent"] = get_agent_features(
            new_agents_array, av_id, num_historical_steps=11
        )
...
def batch_process9s_transformer(dir_name, output_dir, num_workers=2):
    ...
    func = partial(wm2argo, output_dir=output_dir, dir_name=dir_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="/home/haoweis/trajdata_smart/SMART/data/waymo/scenario/validation",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/haoweis/trajdata_smart/SMART/data/waymo_processed/scenario/validation",
    )
